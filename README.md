...
KnowHub 内置了微信机器人，基于 iLink API 实现，无需额外部署。

**首次登录**：启动后端后，终端会显示一个二维码，用微信扫码即可完成绑定。登录凭证会自动保存，后续重启无需重复扫码。
...
### Q: 微信机器人怎么配置？
A: 无需额外配置！启动后端后，终端会显示二维码，用微信扫码即可。登录凭证自动保存，后续重启无需重复扫码。详见上面的「微信机器人配置」。
